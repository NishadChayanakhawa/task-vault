var taskProcessing = (function() {
	//csrf token
	var csrfToken;
	//api path
	const API_PATH = "/api/task";
	//record name
	const RECORD_NAME = "Task";

	//common xpaths
	var xpaths = {
		"record": {
			"template": "#recordListTemplate",
			"placeHolder" : "div#recordPlaceholder",
			"table": {
				"table": "table#recordTable",
				"body": "div#recordPlaceholder",
	...
	var showDeleteConfimationModal = function(event) {
		...
		$("h5[id^='recordIdentifier_" + recordId).each(function() {
	...
	var loadRecords_success = function(records) {
		...
		$(xpaths.record.placeHolder).html("");
		$(xpaths.record.template).tmpl(records).appendTo(xpaths.record.placeHolder);
	};
